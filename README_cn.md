...
基于 100,000 次迭代的全面性能测试：

| 操作类型 | std::format | toy::format | 性能提升 |
|---------|-------------|-------------|----------|
| 整数格式化 | 6709μs | 2517μs | **62% 更快** |
| 浮点数格式化 | 7910μs | 5929μs | **25% 更快** |
| 字符串格式化 | 3730μs | 2250μs | **40% 更快** |
| 指针格式化 | 1242μs | 555μs | **55% 更快** |
| 多参数格式化 | 7236μs | 5106μs | **29% 更快** |
| 复杂格式说明符 | 8108μs | 5489μs | **32% 更快** |
| 内存分配 | 1452μs | 978μs | **33% 更快** |
| 总体吞吐量 | 7.88M ops/sec | 12.87M ops/sec | **63% 更高** |

### 关键性能亮点：

**速度改进：**
- **基础类型**：整数、浮点数和字符串操作整体快 25-62%
- **复杂操作**：多参数和复杂格式化快 29-40%
- **内存效率**：内存分配快 33%，内存使用量相当
- **实际场景**：实际使用场景快 24-26%

**吞吐量优势：**
- **每秒操作数**：12.87M vs 7.88M (吞吐量高 63%)
- **一致性能**：在所有测试场景中保持优越性能
- **边界情况**：空字符串处理快 23%，长字符串处理快 57%

**专业性能：**
- **UTF-8 字符串**：处理速度快 38%
- **时间/时长**：内置优化的 chrono 格式化
- **混合类型**：复杂混合类型格式化快 25-33%

性能测试表明 `gr::toy::format` 在所有类别中持续优于 `std::format`，使其成为性能关键应用的理想选择。
